/**
 * @file win32.c
 * @brief Vulkan win32 extension module
 */
/*****************************************************************************
 * Copyright © 2018 Marvin Scholz, Niklas Haas
 *
...
static int Open (vlc_object_t *obj)
{
    ...
    int ret = vk_CreateInstance(vk, VK_KHR_WIN32_SURFACE_EXTENSION_NAME);
    if (ret != VK_SUCCESS)
        goto error;
    ...
    VkResult res = vkCreateWin32SurfaceKHR(inst, &winfo, NULL, &vk->surface);
    if (res != VK_SUCCESS)
        goto error;
...
vlc_module_begin ()
    set_shortname (N_("VkWin32"))
    set_description (N_("Win32 extension for Vulkan"))
